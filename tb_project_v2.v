...
`define MAX_MISMATCHES 121

// file for output
// this is only useful if decoding is done all the way through (e.g. milestone 1 is used)
`define OUTPUT_FILE_NAME "tb_motorcycle.ppm"
//`define OUTPUT_FILE_NAME "tb_cat_tb.ppm"
//`define OUTPUT_FILE_NAME "tb_panda.ppm"

// file for comparison
// to test milestone 2 independently, use the .sram_d1 file to check the output
`define VERIFICATION_FILE_NAME "motorcycle.sram_d0"
//`define VERIFICATION_FILE_NAME "cat_q0.sram_d0" // for milestone1
//`define VERIFICATION_FILE_NAME "cat_q0.sram_d1" // m2
//`define VERIFICATION_FILE_NAME "motorcycle.sram_d1" // m2
//`define VERIFICATION_FILE_NAME "panda.sram_d0" // for milestone1

//// for milestone 1
//`define INPUT_FILE_NAME "motorcycle.sram_d1"
//`define INPUT_FILE_NAME "panda.sram_d1"
//`define INPUT_FILE_NAME "cat_q0.sram_d1"	// for milestone1

//// for milestone 2
//`define INPUT_FILE_NAME "cat_q0.sram_d2"
...
initial begin
	...
	//project.m1_start = 1'b1;
	//uut.project.m1_start = 1'b1;
	//uut.project.m2_start = 1'b1;

	//$write("uut.top_state: %d\n\n", uut.top_state);
	//$write("Milestone1 started? %d\n\n", uut.project.m1_start);
	//$write("Milestone2 started? %d\n\n", uut.project.m2_start);


	wait (uut.top_state != 0);	//this assumes S_IDLE is the first in the list where the states are enumerated
	Push_buttons = 4'hf;
	
	//wait (uut.project.M1_unit.M1_state != 2);
	//wait (uut.project.m1_start == 1);
...
always @ (posedge Clock_50) begin
	if (uut.SRAM_we_n == 1'b0) begin	//signal names within project (instantiated as uut) should match here, assuming names from experiment4a
		...
		/*
		$write("SRAM: wrote value %d (%x hex) to location %d (%x hex), should be value %d (%x hex)\n",
		...
		*/
		...
		/*
		if (SRAM_ARRAY[uut.SRAM_address] == uut.SRAM_write_data) begin
			$write("GOOD! value %d (%x hex) to location %d (%x hex), and wanted %d (%x hex)\n",
				uut.SRAM_write_data, uut.SRAM_write_data, uut.SRAM_address, uut.SRAM_address,
				SRAM_ARRAY[uut.SRAM_address], SRAM_ARRAY[uut.SRAM_address]);
		end
		*/
		...
		if (SRAM_ARRAY_write_count[uut.SRAM_address] != 1 && warn_multiple_writes_to_same_location < `MAX_MISMATCHES) begin
			$write("warning: written %d times (value: %d) to location %d (%x hex), sim time %t\n",
				SRAM_ARRAY_write_count[uut.SRAM_address], uut.SRAM_write_data, uut.SRAM_address, uut.SRAM_address, $realtime);
